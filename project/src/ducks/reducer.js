...
export default function reducer(state = initialState, action){
    switch(action.type){
        ...
        return Object.assign({}, state, {isLoading: false, allposts: action.payload});
        case GET_ALL_YOUR_POSTS:
...
export function getAllYourPosts(){
    return {
        type: GET_ALL_YOUR_POSTS,
        payload: axios.get('/api/getyourposts').then(response => response.data)
    }
}
...
export function createPost(){
    axios.post('/api/post', {
                story_title: type_title,
                story_text: props.type_story,
